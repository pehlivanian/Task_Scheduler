...
scheduler<F>::set_futures()
{
      ...
      ++i)
    {
      try
        ...
        {std::cerr << "future for task : " << i << " already retreived.\n"; /* future retreived */ }
    }
}
...
scheduler<F>::set_initial_node(bool checkDAG)
{
    ...
    initial_node = (run_levels.end() != initIt) ? *initIt : -1;
    }
}
...
template<typename F>
std::vector<int>
scheduler<F>::ELS_delta(const Graph& gr) const
{
...
template<typename F>
bool
scheduler<F>::is_DAG(const Graph& g) const
{
    Vertexcont toporder;
...
scheduler<F>::set_run_levels()
{
    ...
    auto run_order_sorter = [this](int l, int r){ return run_levels[l] < run_levels[r];};
...
template<typename F>
std::list<Vertex>
scheduler<F>::get_sources(const Vertex& v) const
{
    std::list<Vertex> r;
    Vertex v1;
    inIt j, j_end;
...
{
    // for_each(run_order.begin(), run_order.end(), [this](auto i){if (th[i].joinable()) { th[i].join(); }});
